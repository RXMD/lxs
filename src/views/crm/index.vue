<template>
  <div>
    <transition name="el-zoom-in-center">
      <div class="YZ-preview-main">
        <div class="filter-container">
          ...
          <el-button v-if="false" class="filter-item" type="primary" icon="el-icon-edit" @click="edit({}, 'add')">
            新增
          </el-button>
        </div>
        <div class="dynamic-form-main w-100">
          <el-table ref="table" v-loading="listLoading" :data="userList" border class="w-100 YZ-common-table bd-table table-border-bottom" size="mini" stripe highlight-current-row @row-click="rowClick">
            ...
            <el-table-column align="center" prop="sexText" label="性别" />
            <el-table-column align="center" prop="eMail" label="邮箱" />
            <el-table-column align="center" label="操作" width="300" fixed="right">
              <template slot-scope="scope">
                <el-button v-if="false" size="mini" type="text" @click="edit(scope.row, 'edit')">编辑</el-button>
                <el-button v-if="false" size="mini" type="text" class="YZ-table-delBtn" @click="handleDel(scope.row.id, scope.$index)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
    ...
    <!-- <UserEdit :showDidlog="showDidlog" @closed="closed" :currentItem="currentItem" :type="dialogType"></UserEdit> -->
  </div>
</template>
...
import { getList, getInfo } from '@/api/customer'
export default {
  name: 'User',
  components: {
    // UserEdit
