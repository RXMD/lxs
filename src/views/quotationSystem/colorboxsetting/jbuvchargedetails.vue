<template>
  <transition name="el-zoom-in-center">
    <el-tabs type="border-card" class="w-100 h-100">
      <el-tab-pane label="后工局部UV参数设置" class="w-100 h-100">
        <div ref="view" class="YZ-preview-main">
          <div class="w-100 pd-0">
            ...
            >
              ...
              <el-table-column align="center" prop="dFilmAmount" label="菲林费(元/㎡)">
                <template slot-scope="scope">
                  <el-input v-model.number="scope.row.dFilmAmount" v-Empty-Zero v-Float oninput="value=value.replace(/[^0-9.]/g,'')" size="mini" />
                </template>
              </el-table-column>
              <el-table-column align="center" prop="dMinFilmAmount" label="菲林费最低收费(元)	">
                <template slot-scope="scope">
                  <el-input v-model.number="scope.row.dMinFilmAmount" v-Empty-Zero v-Float oninput="value=value.replace(/[^0-9.]/g,'')" size="mini" />
                </template>
              </el-table-column>
              <el-table-column align="center" label="操作" width="80" fixed="right">
                <template slot-scope="scope">
                  <el-button size="mini" type="text" @click="copy(scope.row, scope.$index)">复制</el-button>
                  <el-button size="mini" type="text" class="YZ-table-delBtn" @click="remove(scope.row, scope.$index, 'Ele_ColorBoxSetting_AfterProcess_LocalUV')">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
...
export default {
  ...
  methods: {
    getList() {
      this.tableData = []
      this.loading = true
      Api.GetColorBoxSettingAfterProcessLocalUVListData(this.uGuid1)
        .then(res => {
          if (res.code == 200) {
            this.tableData =
                            res.data.map(item => {
    ...
    sumbit() {
      ...
      Api.SaveColorBoxSettingAfterProcessLocalUV({
        LocalUVDetailsList: this.tableData,
