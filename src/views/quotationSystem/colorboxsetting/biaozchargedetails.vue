          ...
          <div class="w-100 pd-0">
            ...
            >
              ...
              <el-table-column align="center" prop="dTemplateAmount" label="模板费（元">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.dTemplateAmount" v-Empty-Zero v-Float class="w-100" oninput="value=value.replace(/[^0-9.]/g,'')" size="mini" />
                </template>
              </el-table-column>
...
export default {
  ...
  methods: {
    getList() {
      this.tableData = []
      this.loading = true
      Api.GetColorBoxSettingAfterProcessMountedPaperListData()
        .then(res => {
