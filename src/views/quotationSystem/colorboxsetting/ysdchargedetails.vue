<template>
  <transition name="el-zoom-in-center">
    <el-tabs type="border-card" class="w-100 h-100">
      <el-tab-pane label="后工易撕带参数设置" class="w-100 h-100">
            ...
            >
              <el-table-column align="center">
                <template slot="header">
                  <div class="d-flex ai-c jc-ce">易撕带材料</div>
                </template>
                ...
                <el-table-column align="center" label="操作" width="120" fixed="right">
                  <template slot-scope="scope">
                    ...
                    <el-button size="mini" type="text" class="YZ-table-delBtn" @click="remove(scope.row, scope.$index, 'Ele_ColorBoxSetting_AfterProcess_DoubleFilm')">
                      删除
                    </el-button>
                  </template>
                </el-table-column>
...
export default {
  ...
  methods: {
    getList() {
      this.tableData = []
      this.loading = true
      Api.GetColorBoxSettingAfterProcessEasyTearTapeListData(this.uGuid1)
        .then(res => {
          if (res.code == 200) {
            this.tableData =
                            res.data.map(item => {
    ...
    sumbit() {
      ...
      Api.SaveColorBoxSettingAfterProcessEasyTearTape({
        EasyTearTapeDetailsList: this.tableData,
