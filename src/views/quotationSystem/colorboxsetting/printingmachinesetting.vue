  ...
  <transition name="el-zoom-in-center">
    <div ref="view" class="YZ-common-layout pd-0">
      ...
      <el-dialog v-DialogDrag width="70%" title="选择数据" :visible.sync="editVisible" append-to-body :close-on-click-modal="false" @close="editVisible = false">
        <printing-machine-setting-edit :id="id" :is-dialog="true" @close="closeDialog" @refresh="Refresh" />
      </el-dialog>
    </div>
  </transition>
</template>
...
import PrintingMachineSettingEdit from './printingmachinesettingEdit'
export default {
  name: 'Printingmachinesetting',
  components: {
    PrintingMachineSettingEdit
  },
  props: {
    tab: String
  },
  ...
  watch: {
    tab(v) {
      if (v == 'printingMachineParameterSettings') {
        this.getTableH()
        this.initData()
        this.doLayout()
      }
  ...
  methods: {
    ...
    getList() {
      ...
      Api.GetColorBoxSettingPrintMachineSettingListData()
        .then(res => {
          if (res.code === 200) {
    ...
    handleDel(id, index) {
      this.$confirm('是否删除数据？', '提示', { type: 'warning' })
        .then(() => {
          Api.DeleteColorBoxSettingPrintMachineSetting(id).then(res => {
            if (res.code == 200) {
